...
func (s *PostgresServiceGateway) GetAllServicesMatchingConsumer(consumer models.Consumer) ([]models.Service, error) {
	query := `
		SELECT s.id, s.name, s.target_url, s.path, s.description, s.created_at, s.updated_at, s.health_check_enabled, s.health, r.id, r.path, r.description, r.created_at, r.updated_at, r.method
		FROM service s
		LEFT JOIN route r
		ON s.id=r.service_id
		LEFT JOIN consumers_has_services cs
		on s.id = cs.service_id
		WHERE cs.consumer_id=$1
		ORDER BY s.id
	`

	rows, err := s.Conn.Query(context.Background(), query, consumer.Id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
